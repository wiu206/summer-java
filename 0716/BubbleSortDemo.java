...
/*
 * bubbleSort 外層迴圈 Time Complexity: O(n)
 * 說明：最多進行 n-1 輪比較，每輪將最大值浮到後面
 */

/*
 * bubbleSort 內層迴圈 Time Complexity: O(n - i - 1)
 * 說明：每一輪比較次數遞減，總和為 n(n-1)/2，故最壞為 O(n²)
 */

/*
 * bubbleSort 交換操作 Time Complexity: O(1)
 * 說明：每次交換為常數時間操作
 */

/*
 * cocktailSort 外層 while 迴圈 Time Complexity: O(n²)
 * 說明：最壞情況下會重複左右掃描，每輪包含兩次 O(n) 掃描
 */

/*
 * cocktailSort 左→右 & 右→左 for 迴圈 Time Complexity: O(n)
 * 說明：每次從 left 到 right、或從 right 到 left，最多 n 次比較
 */
